...
import { getMockData } from '@/components/admin/mock/MOCK_prdGuide';
...
const PrdGuide = () => {
  ...
  useEffect(() => {
    const fetchCategory3Options = async () => {
      ...
      try {
        ...
        // 임시 데이터 (실제 API 호출로 교체 필요)
        let data = [];
        if (category1 === 'Electrolux' && category2 === 'stick') {
          data = [
            { value: 'well-q6', label: 'Well Q6' },
            { value: 'well-q7', label: 'Well Q7' },
            { value: 'well-q8', label: 'Well Q8' }
          ];
        } else if (category1 === 'AEG' && category2 === 'consumable') {
          data = [
            { value: 'filter', label: '필터' },
            { value: 'dustbag', label: '먼지봉투' }
          ];
        }
  ...
  useEffect(() => {
    const fetchCategory4Options = async () => {
      ...
      try {
        ...
        // 임시 데이터 (실제 API 호출로 교체 필요)
        let data = [];
        if (category1 === 'Electrolux' && category2 === 'stick' && category3 === 'well-q6') {
          data = [
  ...
  const columns = [
    ...
    },
    {
      ...
      width: '30%',
      render: (text, record) => (
        <ColumnLink onClick={() => navigate(`/mng/dwn/manual/${record.id}`)}>
          {text}
        </ColumnLink>
      ),
    },
  ...
  return (
    <div>
      ...
      <DataTable 
        ...
        rightButtons={<CommonButton onClick={() => navigate('/mng/dwn/manual/new')}>작성하기</CommonButton>}
      />
    </div>
  );
};
