...
const FindPW = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!formData.id || !formData.name || !formData.hp2 || !formData.hp3 || !formData.emailId || !formData.emailDomain) {
      message.error('모든 필드를 입력해주세요.');
      return;
    }
    message.success('임시비밀번호 발급 요청이 전송되었습니다.');
  };
  ...
  return (
    <Container>
      <ContentsWrap>
        <FormWrap>
          <FormTitle>임시비밀번호 발급 방법을 선택해주세요.</FormTitle>
          
          <FindWrap>
            <PhoneAuth>
              <ul className="top_tit">실명인증으로 임시비밀번호 발급</ul>
              <ul className="txt01">
            ...
            <InfoSection>
              <ul className="top_tit">내 회원정보로 임시비밀번호 발급</ul>
